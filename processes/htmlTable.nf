    ...
    if ( ${params.split_html} )
    {
        projects = Channel.fromPath("${launchDir}/config/sample_annotations.tsv")
            ...
            .map { line ->
                ...
                tuple(filename, projects)
            }
        
        for (prj in projects.projects.uniq)  // MAKE UNIQUE??
        {
            def tmp_prj = projects.filter{ project.projects == prj}
            filterSampleAnnot(tmp_prj.filename, prj)
        }
    }
    else 
    {
        projects.file_name.to_csv("samples.tsv", sep = "\t") //FIX
    }
}
